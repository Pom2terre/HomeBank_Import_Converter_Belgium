...
$bundleDir = Join-Path (Split-Path -Parent $sourceRoot) "dist\HomeBankConverterGUI"
$releaseRoot = Join-Path (Split-Path -Parent $sourceRoot) "releases"
...
$iconFile = Join-Path (Split-Path -Parent $sourceRoot) "12218940.ico"
$templatePath = Join-Path $sourceRoot "HomeBankConverterGUI.iss"

if (-not (Test-Path $bundleDir)) {
    Write-Host "Bundle not found at $bundleDir. Building it first..." -ForegroundColor Yellow
    & (Join-Path (Split-Path -Parent $sourceRoot) "build_windows_bundle.ps1")
    if (-not (Test-Path $bundleDir)) {
...
$issTemplate = $issTemplate.Replace('"C:\Dev\Homebank-New\dist\HomeBankConverterGUI"', "`"$bundleDir`"")
$issTemplate = $issTemplate.Replace('"C:\Dev\Homebank-New\releases"', "`"$releaseRoot`"")
$issTemplate = $issTemplate.Replace('"C:\Users\username\OneDrive\Programming\Python\Homebank New\12218940.ico"', "`"$iconFile`"")
